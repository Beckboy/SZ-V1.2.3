...
//    static final int ITEM_LOCATION = 3;
    ...
    private JumpToGroupChatDetails jumpActivity;
    public SendMyMsgBroadcasd sendMyMsgBroadcasd;
...
//    protected int[] itemIds = { ITEM_TAKE_PICTURE, ITEM_PICTURE, ITEM_LOCATION };
    ...
    public void onAttach(Activity activity) {
        ...
        mContext = activity;
        jumpActivity = (JumpToGroupChatDetails) activity;
        sendMyMsgBroadcasd = (SendMyMsgBroadcasd) activity;
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        toChatUserId = fragmentArgs.getString(EaseConstant.EXTRA_GROUPCHAT_ID);
        toChatUserName = fragmentArgs.getString(EaseConstant.EXTRA_GROUPCHAT_TITLE);
        userName = fragmentArgs.getString(EaseConstant.EXTRA_USER_NICKNAME);
        userAvatar = fragmentArgs.getString(EaseConstant.EXTRA_USER_AVATAR);
    ...
    protected void initView() {
        ...
        inputMenu.setChatInputMenuListener(new ChatInputMenuListener() {
            ...
            @Override
            public void onSendMessage(String content,Object[] user) {
                sendTextMessage(content,user);
            }
        ...
        jumpActivity.startActivity(1,null);

    }

    public void setGroupChatCount(int count){
        if (titleBar != null){
            titleBar.setTitle(toChatUserName,"("+count+")");
        }
    }
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) {
        	...
        	titleBar.setRightImageResource(R.drawable.selector_bg_groupchat_detail_info);
    ...
    protected void setListItemClickListener() {
        messageList.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
            ...
            public void onUserAvatarLongClick(String username) {
                ...
                bundle.putString(EaseConstant.BROAD_ACTION_AVATAR_LONGCLICK_USERID,username);
                intent.putExtras(bundle);
                intent.setAction(EaseConstant.BROAD_ACTION_AVATAR_LONGCLICK);
    ...
    public void onStart() {
        super.onStart();
            ...
            intentFilter.addAction(EaseConstant.BROAD_ACTION_AVATAR_LONGCLICK);
            mContext.registerReceiver(receiverAvaterLongClickBroad, intentFilter);
    }

    @Override
    public void onDestroy() {
        mContext.unregisterReceiver(receiverAvaterLongClickBroad);
        super.onDestroy();
     ...
     * 接收更新列表消息的广播
     */
    BroadcastReceiver receiverAvaterLongClickBroad=new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action=intent.getAction();
            switch (action){
                case EaseConstant.BROAD_ACTION_AVATAR_LONGCLICK:
                    Bundle bundle=intent.getExtras();
                    String username = bundle.getString(EaseConstant.BROAD_ACTION_AVATAR_LONGCLICK_USERID);
                    inputMenu.getPrimaryMenu().onTextInsert(username+" ");
                    break;
            }
        }
    };
...
//                break;
    ...
    //send message
    protected void sendTextMessage(String content,Object[] user) {
        if(EaseAtMessageHelper.get().containsAtUsername(content)){
            ...
            if (null != user){
                for (int i= 0;i < user.length;i++){
                    message.setAttribute(user[i].toString(),"有人@我");
                    Log.i("mmmmt",user[i].toString());
                }
            }
            sendMessage(message);
        }
    ...
    protected void sendMessage(EMMessage message){
        ...
        message.setAttribute(EaseConstant.MESSAGE_ATTR_USER_NICKNAME,userName);
        message.setAttribute(EaseConstant.MESSAGE_ATTR_USER_AVATAR,userAvatar);
        if(chatFragmentHelper != null){
            //set extension
        ...
        if(isMessageListInited) {
            messageList.refreshSelectLast();
            String msg = EaseCommonUtils.getMessageDigest(message, mContext);
            sendMyMsgBroadcasd.sendbroad(toChatUserId, msg);
        }
    }

    public interface SendMyMsgBroadcasd{
        void sendbroad(String hx_roomId, String msg);
    }
    ...
    protected void toGroupDetails() {
        if (chatType == EaseConstant.CHATTYPE_GROUP) {
            Bundle bundleToGroupChatDetails = new Bundle();
            bundleToGroupChatDetails.putString(EaseConstant.GROUPCHAT_USER_ID,toChatUserId);
            jumpActivity.startActivity(0, bundleToGroupChatDetails);
        }else if(chatType == EaseConstant.CHATTYPE_CHATROOM){
        	if(chatFragmentHelper != null){
    ...
    public interface JumpToGroupChatDetails{
        void startActivity(int what, Bundle bundle);
    }
