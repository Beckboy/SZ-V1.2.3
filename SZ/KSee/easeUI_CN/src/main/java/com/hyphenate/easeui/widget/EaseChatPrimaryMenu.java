...
public class EaseChatPrimaryMenu extends EaseChatPrimaryMenuBase implements OnClickListener {
    ...
    private Map<String,String> contractMap = new TreeMap<>();
...
//                    faceLayout.setVisibility(View.GONE);
                    ...
                    editText.setBackgroundResource(R.drawable.icon_chat_line);
                } else {
...
//                    faceLayout.setVisibility(View.VISIBLE);
                ...
                int c = editText.getSelectionEnd();
                String person = "";
                String tt = editText.getText().subSequence(0,c).toString();
                int end = 0;
                if (tt.contains("@")){
                    end = tt.lastIndexOf("@");
                    person = tt.subSequence(end,c).toString();
                }
                if (contractMap.containsKey(person)){
                    editText.getText().delete(end,c);
                    if (!editText.getText().toString().contains(person)){
                        contractMap.remove(person);
                    }
                }
            }
        ...
        editText.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                ...
                if (actionId == EditorInfo.IME_ACTION_SEND ||
                    ...
                    editText.setText("");
                    listener.onSendBtnClicked(s,getContractMap(s));
                    contractMap.clear();
                    return true;
                }
                else{
...
//         mEditTextContent.setVisibility(View.VISIBLE);
        editText.requestFocus();
        // 打开键盘
        EaseCommonUtils.softInputFromWiddow(activity);
        // buttonSend.setVisibility(View.VISIBLE);
        buttonPressToSpeak.setVisibility(View.GONE);
        if (TextUtils.isEmpty(editText.getText())) {
            buttonMore.setVisibility(View.VISIBLE);
     ...
     * @param text
     */
    private void addContractList(CharSequence text) {
        String tt = text.subSequence(0,text.length()-1).toString();
        if (tt.contains(":")){
            String[] t = tt.split(":");
            contractMap.put(t[1],t[0]);
        }
    }
    ...
    public Object[] getContractMap(String s) {
        if ( null ==  contractMap) return null;
        if ( null == s) return null;
        Object[] user = contractMap.values().toArray();
        for (int i = 0; i < user.length;i++){
            if (!s.contains(user[i].toString())){
                contractMap.remove(user[i]);
            }
        }
        return user;
    }
